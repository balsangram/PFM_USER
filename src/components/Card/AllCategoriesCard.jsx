...
function AllCategoriesCard({
  ...
  const handleAdd = async (e) => {
    ...
    try {
      setLoading(true);
      await customerApi.addToCart(id, 1);
      setCount(1);
    } finally {
      setLoading(false);
    }
    ...
    >
      ...
      <div className="p-4 space-y-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm line-through text-gray-400">₹{mrpPrice}</span>
          <span className="text-xs font-semibold text-green-600 ml-auto">
            Save ₹{mrpPrice - price}
          </span>
        </div>
        ...
        <div onClick={(e) => e.stopPropagation()}>
          {count > 0 ? (
            ...
            <button
              onClick={handleAdd}
              disabled={loading}
              className="w-full mt-2 bg-red-600 hover:bg-red-700 text-white rounded-lg py-2 text-sm font-semibold"
            >
              ADD
            </button>
          )}
